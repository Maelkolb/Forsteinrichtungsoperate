"""Image pre-processing for historical document scans.

Steps (all optional, controlled by PipelineConfig):
  1. Deskew        – straighten pages with up to ±3° of rotation
  2. Contrast      – auto-contrast to compensate for yellowed / faded paper
  3. Sharpen       – mild unsharp-mask to improve ink edge definition

All operations are applied in-place (overwrite the file).
"""
...
def preprocess_page(page_path: Path, cfg: PipelineConfig) -> Path:
    ...
    if cfg.deskew:
        try:
            ...
            log.warning("Deskew failed for %s: %s", page_path.name, exc)

    # ── 2. Contrast enhancement ────────────────────────────────────────
    # autocontrast with a small cutoff clips the darkest / lightest 1 % of
    # pixels, which effectively neutralises yellowed paper backgrounds while
    # keeping ink dark.
    ...
    # ── 3. Sharpening ──────────────────────────────────────────────────
    # A mild unsharp-mask improves the definition of Kurrent strokes and
    # thin table lines without introducing ringing artefacts.
    if cfg.sharpen:
